#!/usr/bin/env bash
#
# run.sh — Closed Single-Agent Loop runner (TDD グリーン化検証)
#
...
#   VERIFY_CMD    各周のエージェント実行後に走らせる品質ゲート。
#                 既定: `npm run verify`（typecheck → lint → test）
...
echo "== Closed Single-Agent Loop (TDD green-ization) =="
