...
依存解決 / 人間判断 / リファクタ / バグ修正 / maker-checker 分離 / 新機能追加 / 分解ヒントなし機能追加 /
長期収束・MEMORY 背骨）を回した。
...
>
> **実リポジトリへの適用は [`docs/apply-to-real-repo.md`](docs/apply-to-real-repo.md)。**
> ポータブル雛形（[`templates/single-agent-loop/`](templates/single-agent-loop/)）のコピー手順＋新セッション用のコピペ・キックオフ指示。
